...
export function PopUp({ productName, productCategory, open, setOpen }) {
  ...
  const sendMail = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Your inquiry has been submitted successfully! A callback will be arranged shortly.');
        setOpen(false);
        router.push("/thankyou");
      } else {
        toast.error('An error occurred: ' + result.message + ' Please try again');
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <DialogHeader className="relative m-0 block">
        <Typography className="font-ibarra text-3xl md:text-4xl" color="blue-gray">
        {productName} 
        </Typography>
      ...
      <DialogFooter>
        <Button onClick={sendMail} className="font-jost rounded-sm border-dotted border border-white hover:scale-110 transform transition duration-500 bg-[#AF8C3E] capitalize font-medium text-base w-full">
          Submit
        </Button>
      </DialogFooter>
    </Dialog>
